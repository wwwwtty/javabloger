...
<@pagePanel cur=parameters.curPage total=parameters.totalPage url=parameters.url limitlimitlimit=limit />     
<#--打印选择页的面板结束-->     
...
<#macro pagePanel cur total limit url curCssClass = "cur_page"><#--curCssClass默认值为cur_page-->     
    ...
    <#if total <= limit>     
        ...
        <@printPage left = 1 right = total curcurcur = cur urlurlurl = url curCssClasscurCssClasscurCssClass = curCssClass />     
    ...
    <#--情况二：总页数大于限制显示页数，这时又分三种情况-->     
        ...
        <#if cur <= l_mid>     
            <@printButton direct="left" valuable = false />     
            <@printPage left = 1 right = limit curcurcur = cur urlurlurl = url curCssClasscurCssClasscurCssClass = curCssClass />     
        ...
        <#elseif (cur > (total - l_mid))>     
            ...
            <@printPage left = (total - limit + 1) right = total curcurcur = cur urlurlurl = url curCssClasscurCssClasscurCssClass = curCssClass />     
            <@printButton direct="right" valuable = false />     
                  
        <#--在中间的情况-->     
        <#else>     
            <@printButton direct="left" valuable = true />     
            <@printPage left = (cur - l_mid + 1) right = (cur + l_mid -1) curcurcur = cur urlurlurl = url curCssClasscurCssClasscurCssClass = curCssClass />     
            <@printButton direct="right" valuable = true />     
        </#if>     
    </#if>     
